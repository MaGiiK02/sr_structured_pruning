...
def vis_saliency_kde(map, zoomin=4):
    ...
    positions = np.vstack([X.ravel(), Y.ravel()])
    pixels = np.vstack([datapoint_x.ravel(), datapoint_y.ravel()])
    print("bef kde",np.min(pixels), np.max(pixels))
    kernel = stats.gaussian_kde(pixels, weights=grad_flat)
...
def prepare_images(hr_path, scale=4, half_input_size= False):
    hr_pil = Image.open(hr_path)
    
    # We use images with half the size to fit them in memory
    if half_input_size: 
        sizex, sizey = hr_pil.size
        hr_pil = hr_pil.resize((sizex // 2, sizey // 2), Image.BICUBIC)
