...
import torch
import cv2
from lam.SaliencyModel.utils import grad_norm, IG_baseline, interpolation, isotropic_gaussian_kernel, _add_batch_one, _remove_batch
...
def Path_gradient(numpy_image, model, attr_objective, path_interpolation_func, cuda=False):
    ...
    for i in range(image_interpolation.shape[0]):
        ...
        result_list.append(quantize(result, rgb_range=255))
    results_numpy = np.asarray(result_list)
